...
<div class="mb-6">
    <div class="flex items-center gap-3">
        ...
        <div>
            ...
            <p class="text-gray-600">Ubah data menu</p>
        </div>
    </div>
</div>

<div class="bg-white rounded-xl shadow-sm max-w-2xl">
    <form action="{{ route('admin.menu.update', $menu) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-4" data-unsaved="true" oninput="this.dataset.dirty='true'" onsubmit="this.dataset.dirty='false'"
          x-data="{
              form: { nama: '{{ old('nama', $menu->nama) }}', kategori_id: '{{ old('kategori_id', $menu->kategori_id) }}', harga: '{{ old('harga', $menu->harga) }}', stok: '{{ old('stok', $menu->stok) }}', deskripsi: '{{ old('deskripsi', $menu->deskripsi) }}', is_tersedia: {{ $menu->is_tersedia ? 'true' : 'false' }}, is_best_seller: {{ $menu->is_best_seller ? 'true' : 'false' }}, is_new: {{ $menu->is_new ? 'true' : 'false' }} },
              errors: {},
              previewUrl: null,
              existingFoto: '{{ $menu->foto ? asset("storage/" . $menu->foto) : "" }}',
              validate() {
                  this.errors = {};
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <select x-model="form.kategori_id" name="kategori_id" class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" :class="errors.kategori_id ? 'border-red-500' : 'border-gray-300'" required>
                    ...
                    @foreach($kategoris as $kat)
                        <option value="{{ $kat->id }}" {{ old('kategori_id', $menu->kategori_id) == $kat->id ? 'selected' : '' }}>{{ $kat->nama }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="number" x-model="form.harga" name="harga" class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" :class="errors.harga ? 'border-red-500' : 'border-gray-300'" required>
                <template x-if="errors.harga"><p class="text-red-500 text-xs mt-1" x-text="errors.harga"></p></template>
                @error('harga') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Stok</label>
                <input type="number" name="stok" value="{{ old('stok', $menu->stok) }}" class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 border-gray-300">
                @error('stok') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>
            <div class="md:col-span-2">
                <label class="block text-sm font-medium text-gray-700 mb-1">Deskripsi</label>
                <textarea name="deskripsi" rows="3" class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 border-gray-300">{{ old('deskripsi', $menu->deskripsi) }}</textarea>
            ...
            <div class="md:col-span-2">
                ...
                <div class="mt-2 flex gap-2">
                    ...
                    <template x-if="!previewUrl && existingFoto">
                        <img :src="existingFoto" class="w-32 h-32 object-cover rounded-lg border">
                    </template>
                </div>
            ...
            <div class="md:col-span-2 space-y-2">
                <label class="flex items-center gap-2">
                    <input type="checkbox" x-model="form.is_tersedia" name="is_tersedia" value="1" class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
