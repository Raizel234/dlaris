...
<div class="mb-6">
    <div class="flex items-center gap-3">
        ...
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Edit Bahan</h2>
            <p class="text-gray-600">Ubah data bahan baku</p>
        </div>
    </div>
</div>

<div class="bg-white rounded-xl shadow-sm max-w-3xl">
    <form action="{{ route('admin.bahan.update', $bahan) }}" method="POST" class="p-6 space-y-4" data-unsaved="true" oninput="this.dataset.dirty='true'" onsubmit="this.dataset.dirty='false'"
          x-data="{
              form: { nama: '{{ old('nama', $bahan->nama) }}', satuan: '{{ old('satuan', $bahan->satuan) }}', stok: '{{ old('stok', $bahan->stok) }}', stok_minimum: '{{ old('stok_minimum', $bahan->stok_minimum) }}', harga_beli: '{{ old('harga_beli', $bahan->harga_beli) }}', supplier: '{{ old('supplier', $bahan->supplier) }}', keterangan: '{{ old('keterangan', $bahan->keterangan) }}' },
              errors: {},
              menuAssignments: [],
              init() {
                  @if(isset($menus))
                      const existingMenus = @json($bahan->menus->keyBy('id')->map(fn($m) => $m->pivot->jumlah_per_porsi));
                      this.menuAssignments = {{ $menus->map(fn($m) => ['id' => $m->id, 'nama' => $m->nama, 'jumlah' => ''])->toJson() }};
                      this.menuAssignments.forEach(m => {
                          if (existingMenus[m.id] !== undefined) {
                              m.jumlah = existingMenus[m.id];
                              m.selected = true;
                          }
                      });
                  @endif
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="text" x-model="form.supplier" name="supplier" class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 border-gray-300">
            ...
            <div>
                ...
                <input type="number" x-model="form.stok" name="stok" step="0.01" class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" :class="errors.stok ? 'border-red-500' : 'border-gray-300'" required>
            ...
            <div>
                ...
                <input type="number" x-model="form.stok_minimum" name="stok_minimum" step="0.01" class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" :class="errors.stok_minimum ? 'border-red-500' : 'border-gray-300'" required>
            ...
            <div>
                ...
                <input type="number" x-model="form.harga_beli" name="harga_beli" step="0.01" class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" :class="errors.harga_beli ? 'border-red-500' : 'border-gray-300'" required>
            ...
            <div class="md:col-span-2">
                ...
                <textarea x-model="form.keterangan" name="keterangan" rows="3" class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 border-gray-300">{{ old('keterangan', $bahan->keterangan) }}</textarea>
