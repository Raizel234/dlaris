...
<div class="mb-6">
    <div class="flex items-center gap-3">
        ...
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Tambah Meja</h2>
            <p class="text-gray-600">Buat meja baru</p>
        </div>
    </div>
</div>

<div class="bg-white rounded-xl shadow-sm max-w-lg">
    <form action="{{ route('admin.meja.store') }}" method="POST" class="p-6 space-y-4" data-unsaved="true" oninput="this.dataset.dirty='true'" onsubmit="this.dataset.dirty='false'">
        @csrf
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Nomor Meja <span class="text-red-500">*</span></label>
            <input type="text" name="nomor_meja" value="{{ old('nomor_meja') }}" class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 @error('nomor_meja') border-red-500 @enderror" required>
